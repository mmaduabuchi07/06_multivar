...
- Based on Y (outcome) existence:
    - Supervised (labels, you need to specify the y)
    - Unsupervised  

- If Y exists, based on its type:  
    - Categorical: classification  
    - Numerical: regression  
  Sometimes we can change continuose data to categorical.
...
# these two x are hight un correlated.
...
# these two x are hight  correlated. will have multicolinearity problem.
...
lm_dayl.apr %>%
  bind_rows(lm_prp.oct, 
            lm_dayl.prcp,
            lm_daylapr.daylmay) %>% #change the order in dataset is easier than do so in ggplot
...
- Dimensionality reduction  # reduce colunms or X 
...
Exam: use of PCA - to decrease dimension and have uncorelated variables
...
lm_pca <- lm(strength_gtex ~ PC1 + PC2 + PC4 + PC4,
